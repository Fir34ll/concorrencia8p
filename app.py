    ...
    try:
        with timeout_lock(lock):
            if not queue or request.sid != queue[0]['sid']:
                emit('error', {'message': 'Não é sua vez ainda, calma lá !'}, room=request.sid)
                return
            ...
            for event in events:
                if event['id'] == event_id and event['slots'] > 0:
                    ...
                    return
            emit('error', {'message': 'Não possui mais vagas, escolha outro evento'}, room=request.sid)
    except TimeoutError:
        emit('error', {'message': 'Sistema ocupado, tente novamente'}, room=request.sid)
